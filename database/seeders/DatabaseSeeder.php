...
use App\Models\AlarmPartition; // <-- Asegúrate de que el modelo exista o se use DB
use App\Models\GpsDevice;
use App\Models\ServicePlan;
use App\Models\Driver;
use Illuminate\Support\Facades\DB; // Para insertar particiones si no hay modelo
...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 3. SIA CODES (Resumido para velocidad)
        $codes = [
        ...
        // 4. CLIENTE (Agregado 'status' para evitar error de vista)
        ...
        // PARTICIONES (Usamos DB directo si no tienes el modelo creado)
        ...
        // 6. CONDUCTOR Y GPS (Con 'full_name' y 'status' que faltaban)
        $driver = Driver::create([
            'customer_id' => $customer->id,
            'first_name' => 'Juan',
            'last_name' => 'Conductor',
            'full_name' => 'Juan Conductor', // <--- Requerido por OrderBy
            'phone' => '0412-0000000',
            'status' => 'active', // <--- Requerido por Where
            'is_active' => true
        ]);
