<div class="flex flex-col md:flex-row h-full bg-neutral-900 text-gray-100 overflow-hidden">
    ...
    <div class="w-full md:w-1/2 flex flex-col border-r border-gray-800">
        ...
        <div class="p-6 space-y-4 flex-1 overflow-y-auto">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="bg-gray-800/50 p-3 rounded border border-gray-700/50">
                    <span class="block text-xs text-gray-500 uppercase mb-1">Último Reporte</span>
                    <span class="text-sm font-mono text-white">{{ \Carbon\Carbon::parse($device->last_connection)->diffForHumans() }}</span>
                </div>
            </div>

            <div class="mt-4 pt-4 border-t border-gray-800">
                <h4 class="text-xs font-bold text-gray-500 uppercase mb-3">Comandos Remotos</h4>
    ...
    <div class="w-full md:w-1/2 bg-black/40 flex flex-col">
        <div class="p-6 border-b border-gray-800">
            <h3 class="font-bold text-white flex items-center gap-2"><i class="fas fa-route text-blue-500"></i> Historial</h3>
        </div>
        <div class="p-6 flex-1">
            <div class="space-y-5">
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Inicio</label>
                    <div class="relative">
                        <input type="text" id="start_date" class="datepicker w-full bg-gray-800 border border-gray-600 rounded p-3 pl-10 text-white text-sm focus:border-blue-500 outline-none" value="{{ now()->startOfDay()->format('Y-m-d H:i') }}">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="end_date" class="datepicker w-full bg-gray-800 border border-gray-600 rounded p-3 pl-10 text-white text-sm focus:border-blue-500 outline-none" value="{{ now()->endOfDay()->format('Y-m-d H:i') }}">
                        <i class="fas fa-calendar absolute left-3 top-3.5 text-gray-500"></i>
                    </div>
                </div>
                <button onclick="fetchHistory({{ $device->id }})" id="btn-history" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded shadow-lg shadow-blue-900/50 transition flex items-center justify-center gap-2 mt-4">
